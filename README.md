...
URL | 요청 | 설명
:--: | :--: | :--:
/cultureInfos?category={category}&sort={sort}&page={page} | GET |  문화생활 전체 목록 조회(최신순(new), 인기순(popular), 
기본값: 최신순(new)), 카테고리에 맞는 문화생활 조회
/cultureInfos/search?query={query} | GET | 검색어`query`에 맞는 제목 조회
/cultureInfos/{title} | GET | 검색어`title`에 맞는 문화생활 조회
